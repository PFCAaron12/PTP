...
export function MasterAndStemsUpload() {
    ...
    const handleMasterChange = (e) => {
       ...
       const uploadTask = uploadBytesResumable(storageRef, master.name);
        uploadTask.on(
          ...
          const handleUpload = () => {
            const uploadFiles = [];
            files.forEach((file) => {
              const audioRef = ref(storage, `Other/Exclusive/${uid}/${title}/${file.name}`);
     ...
     async function Firestore () {
      handleUpload();
    const usersDocRef = doc(db, `Other/Exclusive/${uid}/${title}`);
    ...
    function Display () {
      ...
      getDownloadURL(ref(storage, `HipHop/Exclusive/${uid}/${title}/${master.name}`))
        .then((url) => {
